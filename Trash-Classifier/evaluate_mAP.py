...
def get_mAP(Yolo, dataset, score_threshold=0.25, iou_threshold=0.50, TEST_INPUT_SIZE=TEST_INPUT_SIZE):
    ...
    for index in range(dataset.num_samples):
        ...
        for bbox in bboxes:
            ...
            bbox = xmin + " " + ymin + " " + xmax + " " +ymax
            #print("json_pred[gt_classes.index(class_name)] :  ", json_pred[gt_classes.index(class_name)])
            
            #print("class_name :  ", class_name)
            if class_name in gt_classes:
                #print("gt_classes.index(class_name) :  ", gt_classes.index(class_name))
                json_pred[gt_classes.index(class_name)].append({"confidence": str(score), "file_id": str(index), "bbox": str(bbox)})
            else:
                classnotfoundcounter += 1
                notfoundclasslist.append(class_name)
    ...
    with open("mAP/results.txt", 'w') as results_file:
        ...
        print("\n\nclassnotfoundcounter :  ",classnotfoundcounter)
        print("\n\nList of Classes not Found :  ",notfoundclasslist)
